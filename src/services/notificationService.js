...
import * as InternalMessageType from '../messages/internalMessageTypes'
...
export default class NotificationService {
	...
	static async open(notification) {
		...
		const getPopup = async () => {
			try {
				...
				if (typeof browser !== 'undefined') {
					...
					const win = window.open(url, 'MixinWalletPrompt', `width=${width},height=${height},resizable=0,top=${middleY},left=${middleX},titlebar=0`);
		...
		await InternalMessage.payload(InternalMessageType.SET_PROMPT, JSON.stringify(notification)).send();
